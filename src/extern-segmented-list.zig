//! a version of std.SegmentedList which can be used in extern structs
...
pub fn SegmentedList0(comptime T: type) type {
    return SegmentedList(T, 0);
}
...
pub fn SegmentedList(comptime T: type, comptime prealloc_item_count: usize) type {
    return extern struct {
        ...
        pub fn initEmpty() Self {
            return .{ .dynamic_segments = undefined };
        }

        pub fn format(list: Self, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
            var iter = list.constIterator(0);
            var i: usize = 0;
            while (iter.next()) |it| {
        ...
        pub fn deinit(self: *Self, allocator: Allocator) void {
            self.freeShelves(allocator, @intCast(ShelfIndex, self.dynamic_segments_len), 0);
            allocator.free(self.dynamic_segments[0..self.dynamic_segments_len]);
            self.* = undefined;
        }
        ...
        pub fn setCapacity(self: *Self, allocator: Allocator, new_capacity: usize) Allocator.Error!void {
            if (prealloc_item_count != 0) {
                if (new_capacity <= @as(usize, 1) << (prealloc_exp + @intCast(ShelfIndex, self.dynamic_segments_len))) {
        ...
        pub fn growCapacity(self: *Self, allocator: Allocator, new_capacity: usize) Allocator.Error!void {
            ...
            const old_shelf_count = @intCast(ShelfIndex, self.dynamic_segments_len);
            ...
            allocator.free(self.dynamic_segments[0..self.dynamic_segments_len]);
            self.dynamic_segments = new_dynamic_segments.ptr;
            self.dynamic_segments_len = new_dynamic_segments.len;
        }
        ...
        pub fn shrinkCapacity(self: *Self, allocator: Allocator, new_capacity: usize) void {
            if (new_capacity <= prealloc_item_count) {
                const len = @intCast(ShelfIndex, self.dynamic_segments_len);
                self.freeShelves(allocator, len, 0);
                allocator.free(self.dynamic_segments[0..self.dynamic_segments_len]);
                self.dynamic_segments = undefined;
                self.dynamic_segments_len = 0;
                return;
            }
            ...
            if (allocator.resize(self.dynamic_segments[0..self.dynamic_segments_len], new_cap_shelf_count)) {
                // We didn't need the new memory allocation after all.
                // self.dynamic_segments = self.dynamic_segments[0..new_cap_shelf_count];
                self.dynamic_segments_len = new_cap_shelf_count;
...
fn testSegmentedList(comptime prealloc: usize) !void {
    var list = SegmentedList(i32, prealloc).initEmpty();
...
test "std.segmented_list clearRetainingCapacity" {
    var list = SegmentedList(i32, 1).initEmpty();
    defer list.deinit(testing.allocator);
