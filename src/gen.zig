...
fn genErr(comptime fmt: []const u8, args: anytype, err: GenError) GenError {
...
fn fieldTypeName(
    ...
    return genErr("internal error", .{}, error.MissingMessageName);
}
...
pub fn genMessageTest(
    ...
    try ctx.writer.print(
        ...
        \\_ = ctx.deserialize(&{s}.descriptor) catch {{}};
        \\}}
        \\
    , .{message.name.slice()});
}
