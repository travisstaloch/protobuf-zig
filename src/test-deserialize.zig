...
test "examples/only_enum - system protoc" {
    ...
    try testing.expectEqual(1, req.file_to_generate.len);
    try testing.expectEqual(1, req.file_to_generate.cap);
    try testing.expectEqualStrings("examples/only_enum.proto", req.file_to_generate.items[0].slice());
    try testing.expectEqual(1, req.proto_file.len);
    const pf = req.proto_file;
    try testing.expectEqual(1, pf.len);
    const pf0et = pf.items[0].enum_type;
    try testing.expectEqual(1, pf0et.len);
    const pf0et0val = pf0et.items[0].value;
    try testing.expectEqual(4, pf0et0val.len);
    try testing.expectEqualStrings("NONE", pf0et0val.items[0].name.slice());
    try testing.expectEqual(0, pf0et0val.items[0].number);
    try testing.expectEqualStrings("A", pf0et0val.items[1].name.slice());
    try testing.expectEqual(1, pf0et0val.items[1].number);
    try testing.expectEqualStrings("B", pf0et0val.items[2].name.slice());
    try testing.expectEqual(2, pf0et0val.items[2].number);
    try testing.expectEqualStrings("C", pf0et0val.items[3].name.slice());
    try testing.expectEqual(3, pf0et0val.items[3].number);
    const pfscloc = pf.items[0].source_code_info.location;
    try testing.expectEqual(16, pfscloc.len);
}
...
test "examples/only_enum-1 - no deps" {
    ...
    try testing.expectEqual(1, pf0et.len);
    const pf0et0val = pf0et.items[0].value;
    try testing.expectEqual(1, pf0et0val.len);
    try testing.expectEqualStrings("NONE", pf0et0val.items[0].name.slice());
    try testing.expectEqual(0, pf0et0val.items[0].number);
    const pfscloc = pf.items[0].source_code_info.location;
    try testing.expectEqual(7, pfscloc.len);
    try testing.expectEqual(0, pfscloc.items[0].path.len);
    try testing.expectEqual(4, pfscloc.items[0].span.len);
    try testing.expectEqual(1, pfscloc.items[1].path.len);
    try testing.expectEqual(3, pfscloc.items[1].span.len);
    try testing.expectEqual(2, pfscloc.items[2].path.len);
    try testing.expectEqual(4, pfscloc.items[2].span.len);
    try testing.expectEqual(3, pfscloc.items[3].path.len);
    try testing.expectEqual(3, pfscloc.items[3].span.len);
    try testing.expectEqual(4, pfscloc.items[4].path.len);
    try testing.expectEqual(3, pfscloc.items[4].span.len);
    try testing.expectEqual(5, pfscloc.items[5].path.len);
    try testing.expectEqual(3, pfscloc.items[5].span.len);
    try testing.expectEqual(5, pfscloc.items[6].path.len);
    try testing.expectEqual(3, pfscloc.items[6].span.len);
}
...
test "nested lists" {
    ...
    defer req.base.deinit(talloc);
    try testing.expectEqual(1, req.proto_file.len);
    try testing.expectEqual(1, req.proto_file.items[0].enum_type.len);
    try testing.expectEqual(1, req.proto_file.items[0].enum_type.items[0].value.len);
    try testing.expectEqualStrings("field0", req.proto_file.items[0].enum_type.items[0].value.items[0].name.slice());
    try testing.expectEqual(1, req.proto_file.items[0].enum_type.items[0].value.items[0].number);
}
...
test "examples/only_message - no deps" {
    ...
    const pf = req.proto_file;
    try testing.expectEqual(2, pf.len);
    try testing.expectEqual(16, pf.items[0].source_code_info.location.len);
    try testing.expectEqual(21, pf.items[1].source_code_info.location.len);
    const pf0et = pf.items[0].enum_type;
    try testing.expectEqual(1, pf0et.len);
    try testing.expectEqualStrings("SomeKind", pf0et.items[0].name.slice());
    try testing.expectEqual(4, pf0et.items[0].value.len);
    try testing.expectEqualStrings("only_message.proto", pf.items[1].name.slice());
    try testing.expectEqual(1, pf.items[1].dependency.len);
    try testing.expectEqualStrings("only_enum.proto", pf.items[1].dependency.items[0].slice());

    const pf1mt = pf.items[1].message_type;
    try testing.expectEqual(1, pf1mt.len);
    const mt0 = pf1mt.items[0];
    try testing.expectEqualStrings("Person", mt0.name.slice());
    try testing.expectEqual(4, mt0.field.len);
    ...
    try testing.expectEqual(1, mt0.field.items[0].number);
    ...
    try testing.expectEqual(2, mt0.field.items[1].number);
    ...
    try testing.expectEqual(3, mt0.field.items[2].number);
    ...
    try testing.expectEqual(4, mt0.field.items[3].number);
...
test "message with map fields / nested types" {
    ...
    defer req.base.deinit(talloc);
    try testing.expectEqual(1, req.base.unknown_fields.len);
    try testing.expectEqual(1, req.proto_file.len);
    try testing.expectEqual(1, req.proto_file.items[0].message_type.len);
    try testing.expectEqual(1, req.proto_file.items[0].message_type.items[0].nested_type.len);
}
...
test "deser group" {
    ...
    try testing.expectEqual(202, g.data.group_int32);
    try testing.expect(g.data.has(.group_uint32));
    try testing.expectEqual(203, g.data.group_uint32);
}
